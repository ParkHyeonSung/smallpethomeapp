...
export default function PostDetailScreen() {
  ...
  const fallbackId =
    typeof window !== 'undefined'
      ...
      : undefined;
  const postId =
    typeof idParam === 'string' && idParam.length > 0 ? idParam : fallbackId;
  const { width } = useWindowDimensions();
  const isDesktopWeb = Platform.OS === 'web' && width >= 1024;
  const sheetY = useRef(new Animated.Value(SHEET_Y)).current;
  ...
  const activeTag = useMemo(() => tags.find((tag) => tag.id === activeTagId) ?? null, [activeTagId, tags]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setIsLoading(true);
        const [{ data: auth }, nextPost, nextComments, nextTags, likedIds] = await Promise.all([
  ...
  const closeCommentSheet = () => {
    Animated.timing(sheetY, { toValue: SHEET_Y, duration: 180, useNativeDriver: true }).start(() => {
  ...
  const handleToggleLike = async () => {
    ...
    const next = !isLiked;
    setIsLiked(next);
    setPost((prev) => (prev ? { ...prev, like_count: Math.max(prev.like_count + (next ? 1 : -1), 0) } : prev));
    try {
  ...
  const handleDeletePost = () => {
    const runDelete = async () => {
      ...
      try {
        ...
        Alert.alert('삭제 실패', error instanceof Error ? error.message : '게시글 삭제 중 오류가 발생했습니다.');
      } finally {
        setIsDeleting(false);
      }
    };
    ...
    if (Platform.OS === 'web') {
      const confirmed = typeof window !== 'undefined' ? window.confirm('이 게시글을 삭제하시겠습니까?') : false;
  ...
  const handleSaveCommentEdit = async () => {
    ...
    try {
      ...
      setComments((prev) => prev.map((comment) => (comment.id === editingCommentId ? nextComment : comment)));
  ...
  if (isLoading) {
    return <View style={styles.center}><ActivityIndicator size="large" color={colors.primary} /><Text style={styles.muted}>게시글을 불러오는 중입니다...</Text></View>;
  }

  if (!post || !postId) {
    return <View style={styles.center}><Text style={styles.title}>게시글을 찾을 수 없습니다.</Text><Pressable style={styles.pill} onPress={() => router.back()}><Text style={styles.pillText}>뒤로 가기</Text></Pressable></View>;
  }

  return (
    <View style={styles.container}>
      <View style={styles.topBar}>
        <Pressable style={styles.iconButton} onPress={() => router.back()}><Ionicons name="chevron-back" size={20} color={colors.text} /></Pressable>
          ...
          <View style={[styles.media, isDesktopWeb && styles.mediaDesktop]}>
            {post.image_url ? (
              <View style={styles.mediaInner}>
                <Image source={{ uri: post.image_url }} style={styles.image} contentFit="contain" />
                {tags.map((tag) => (
                  <Pressable key={tag.id} style={[styles.marker, { left: `${tag.x_position * 100}%`, top: `${tag.y_position * 100}%` }]} onPress={() => setActiveTagId((prev) => (prev === tag.id ? null : tag.id))} />
                ))}
                {activeTag ? (
                  <View style={styles.tagCard}>
                    <Text style={styles.tagName}>{activeTag.product_name}</Text>
                    <Text style={styles.tagUrl} numberOfLines={1}>{activeTag.product_url}</Text>
                    <Pressable style={styles.smallPill} onPress={handleOpenTagLink}><Text style={styles.smallPillText}>링크 열기</Text></Pressable>
                  </View>
                ) : null}
              </View>
            ) : (
              <View style={[styles.textCover, isDesktopWeb && styles.textCoverDesktop]}>
                <View>
                  ...
                  <Text style={styles.meta}>{formatPostDate(post.created_at)} · {post.is_public ? '공개' : '비공개'}</Text>
                </View>
                <Text style={[styles.textPost, isDesktopWeb && styles.textPostDesktop]}>{post.content}</Text>
                <Text style={styles.metaStrong}>{post.profiles?.nickname ?? '사용자'}의 텍스트 게시글</Text>
              </View>
            )}
          </View>
          ...
          <View style={[styles.side, isDesktopWeb && styles.sideDesktop]}>
            <View style={styles.header}>
              <View style={styles.authorRow}>
                {post.profiles?.avatar_url ? <Image source={{ uri: post.profiles.avatar_url }} style={styles.avatar} contentFit="cover" /> : <View style={styles.avatarFallback}><Text style={styles.avatarFallbackText}>{getInitials(post.profiles?.nickname ?? '사용자')}</Text></View>}
                <View style={styles.authorMeta}><Text style={styles.author}>{post.profiles?.nickname ?? '사용자'}</Text><Text style={styles.muted}>팔로워 {followerCount} · {formatPostDate(post.created_at)}</Text></View>
              </View>
              {isOwnPost ? (
                <View style={styles.row}>
                  <Pressable style={styles.outlinePill} onPress={() => router.push(`/posts/edit/${post.id}`)}><Text style={styles.outlinePillText}>수정</Text></Pressable>
                  <Pressable style={styles.deletePill} onPress={handleDeletePost} disabled={isDeleting}><Text style={styles.deletePillText}>{isDeleting ? '삭제 중...' : '삭제'}</Text></Pressable>
                </View>
              ) : (
                <Pressable style={[styles.followPill, isFollowing && styles.outlinePill, isSubmittingFollow && styles.disabled]} onPress={() => void handleToggleFollow()} disabled={isSubmittingFollow}>
                  <Text style={[styles.followText, isFollowing && styles.outlinePillText]}>{isSubmittingFollow ? '처리 중...' : isFollowing ? '팔로잉' : '팔로우'}</Text>
                </Pressable>
              )}
            </View>

            <View style={styles.row}>
              <Pressable style={styles.metaAction} onPress={() => void handleToggleLike()}><Ionicons name={isLiked ? 'heart' : 'heart-outline'} size={18} color={isLiked ? '#E15A7A' : colors.text} /><Text style={styles.metaStrong}>좋아요 {post.like_count}</Text></Pressable>
              <Pressable style={styles.metaAction} onPress={isDesktopWeb ? undefined : openCommentSheet}><Ionicons name="chatbubble-outline" size={17} color={colors.text} /><Text style={styles.metaStrong}>댓글 {post.comment_count}</Text></Pressable>
              <Text style={styles.metaStrong}>{post.is_public ? '공개' : '비공개'}</Text>
            </View>

            {!post.image_url ? null : <Text style={styles.body}><Text style={styles.bodyAuthor}>{post.profiles?.nickname ?? '사용자'}</Text> {post.content}</Text>}
            ...
            {isDesktopWeb ? (
              <View style={styles.commentPanel}>
                ...
                <ScrollView style={styles.commentScroller} contentContainerStyle={styles.commentList} showsVerticalScrollIndicator={false}>
                  {comments.length === 0 ? <Text style={styles.muted}>아직 댓글이 없습니다.</Text> : comments.map((comment) => (
                    <CommentRow key={comment.id} comment={comment} currentUserId={currentUserId} editingCommentId={editingCommentId} editingCommentDraft={editingCommentDraft} isSubmittingCommentEdit={isSubmittingCommentEdit} menuCommentId={menuCommentId} onChangeEditDraft={setEditingCommentDraft} onCancelEdit={() => setEditingCommentId(null)} onOpenActions={openCommentActions} onSaveEdit={handleSaveCommentEdit} onStartEdit={(target) => { setEditingCommentId(target.id); setEditingCommentDraft(target.content); setMenuCommentId(null); }} onDelete={handleDeleteComment} formatDate={formatPostDate} />
                  ))}
                </ScrollView>
                <View style={styles.inputRow}>
                  <TextInput value={commentDraft} onChangeText={setCommentDraft} placeholder="댓글을 입력하세요." placeholderTextColor={colors.textMuted} style={styles.input} />
                  <Pressable style={[styles.submit, isSubmittingComment && styles.disabled]} onPress={() => void handleSubmitComment()} disabled={isSubmittingComment}><Text style={styles.submitText}>{isSubmittingComment ? '등록 중' : '게시'}</Text></Pressable>
                </View>
              </View>
            ) : null}
          </View>
        ...
        <Modal transparent animationType="none" visible={isCommentSheetVisible} onRequestClose={closeCommentSheet}>
          <View style={styles.overlay}>
            ...
            <Animated.View style={[styles.sheet, { transform: [{ translateY: sheetY }] }]}>
              ...
              <View style={styles.sheetHeader}><Text style={styles.section}>댓글</Text><Pressable onPress={closeCommentSheet}><Ionicons name="close" size={22} color={colors.text} /></Pressable></View>
...
function formatPostDate(value: string) {
  ...
  return now.getFullYear() === date.getFullYear() ? `${month}.${day}` : `${date.getFullYear()}.${month}.${day}`;
}
