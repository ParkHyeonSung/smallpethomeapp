...
function getPrompt(payload: StressAiPayload) {
  return `
너는 소동물 입주 전 환경 체크 결과를 설명하는 도우미다.
...
- 센서 한계, 스마트폰 측정, 참고용 안내는 화면 하단 고정 문구에서 처리하므로 해석 문장에는 쓰지 않는다.
- 반드시 JSON만 반환한다.
...
- 진동은 숫자 점수로 말하지 말고, 안정/약한 흔들림/주의가 필요한 흔들림/강한 흔들림 같은 상태 표현과 패턴 표현으로 설명한다.
